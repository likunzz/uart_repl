...
{
    // arg = argument เป็น parameter ที่ผู้เรียกส่งผ่านทางวงเล็บของfunction
    // int argc = จำนวนของparameter
    // char **argv = array ของ parameter เป็น char* หรือ string
    ...
    int led_no = atoi(argv[1]); // แปลงพารามิเตอร์ตัวที่2
                                // ข้อสังเกตถ ต้องรับมือในกรณีที่ user ป้อนอักขระที่ไม่ใช่ตัวเลย
    printf("LED number = %d\n" , led_no);
    int led_status = 0;
...
void app_main(void)
{
    ...
    // ขั้นตอนการเพิ่ม cmd 1 สร้าง struct ของ cmd 
    // ประกอบด้วย cmd ข้อความช่วยเหลือ และ callback function
    ...
    gpio_set_direction(GPIO_NUM_16,GPIO_MODE_OUTPUT); // กำหนดขา
    ...
    //ขั้นตอนที่2 ลงทะเบียนcmd
    //ด้วย api esp_console_cmd_register
